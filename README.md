...
| `Queue<i>Preset` | `casual` | (none) | Opt-in shortcut for the lenient bundle of defaults: q-start/q-floor 0.4/0.10 (vs 0.6/0.30), no `MaxLiabilityGap` cap, `RelaxTime` 45 s, `RatingWeight` 0.5, and a slightly higher griefing threshold. Of these, only `RelaxTime` has its own per-queue override key (`Queue<i>RelaxTime`, below); the rest — q-start/q-floor, `MaxLiabilityGap`, and `RatingWeight` — are set solely by whether this preset is on (the griefing threshold is separately settable per game type via `<GameType>TeamkillThreshold`). Omit for the standard (stricter) defaults. |
