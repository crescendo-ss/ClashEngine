...
public sealed class RecordingTelemetry : IMatchmakingTelemetry
{
    ...
    public List<(PlayerKey Player, string Queue)> QueueRestored { get; } = new();
    ...
    public List<(PlayerKey Player, Guid MatchId, DateTimeOffset ReturnBy, DateTimeOffset At)> PlayerDepartures { get; } = new();
    ...
    public List<(string Queue, QueueBlockStatus Status)> MatchmakingBlocked { get; } = new();
    ...
    public void OnQueueRestored(PlayerKey player, string queueName, DateTimeOffset at) =>
        QueueRestored.Add((player, queueName));
    ...
    public void OnQueueMatchmakingBlocked(string queueName, QueueBlockStatus status, DateTimeOffset at) =>
        MatchmakingBlocked.Add((queueName, status));
    ...
    public void OnPlayerDeparted(ActiveMatch match, PlayerKey player, DateTimeOffset returnBy, DateTimeOffset at) =>
        PlayerDepartures.Add((player, match.MatchId, returnBy, at));
    public void OnPlayerReturnedToMatch(PlayerKey player, Guid matchId, DateTimeOffset at) =>
        PlayerReturns.Add((player, matchId, at));
