...
| `QualityFunction` | `OrdinalSpread` / `PredictDraw` | `OrdinalSpread` | Engine-wide team-balance metric (the engine holds one instance, shared by every queue). `OrdinalSpread` scores a match by how close the two teams' mean ordinal (`μ − 3σ`) is. `PredictDraw` uses OpenSkill's own win-probability balance (`1 − |win_A − win_B|`), which folds in σ so an uncertain roster reads as closer to even. **Caveat:** the two produce scores on different scales, so the `qStart`/`qFloor` accept bands mean different things when you switch — retune expectations accordingly. Unrecognized values warn and fall back to `OrdinalSpread`. |
