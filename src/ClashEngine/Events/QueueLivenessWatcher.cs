...
/// <remarks>
/// <para>
/// Membership comes from the telemetry composite (<see cref="OnQueueAdded"/> /
/// <see cref="OnQueueRemoved"/> fire for every add/remove path), so the per-position-packet cost
/// for a non-queued player is one dictionary miss. Spectators never reach
