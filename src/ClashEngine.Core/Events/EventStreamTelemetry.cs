...
public sealed class EventStreamTelemetry : IMatchmakingTelemetry
{
    ...
    // KOTH winner promotion, ?autoqueue re-enqueue, and post-match other-queue restoration fire
    // their own telemetry events (so the chat adapter can send a cause-specific DM) instead of
    // OnQueueAdded. For the event stream the *reason* is irrelevant -- the player is now a queue
    // member and the board's count must reflect it -- so all three map to a plain queue.joined. The
    // player is already enqueued before these fire (MatchmakingEngine.ApplyKothReenqueue / the
    // auto-queue and restore loops enqueue before calling telemetry), so ResolveQueue's count is
    // the correct post-add occupancy.
