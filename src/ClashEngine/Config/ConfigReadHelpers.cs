...
/// decide how to default. All helpers take an explicit <see cref="ConfigHandle"/> so the same
/// parser code can read from any conf document (arena.conf or global.conf, plus anything
/// <c>#include</c>'d).</summary>
internal static class ConfigReadHelpers
{
    ...
    public static int? TryReadInt(IConfigManager config, ConfigHandle handle, string key)
    {
        const int Sentinel = int.MinValue;
        int v = config.GetInt(handle, ConfigConstants.Section, key, Sentinel);
    ...
    public static double? TryReadDouble(IConfigManager config, ConfigHandle handle, string key)
    {
        var s = config.GetStr(handle, ConfigConstants.Section, key);
    ...
    public static TimeSpan? TryReadTimeSpan(IConfigManager config, ConfigHandle handle, string key, ClashLog? log, string contextPrefix)
    {
        var s = config.GetStr(handle, ConfigConstants.Section, key);
        if (string.IsNullOrWhiteSpace(s)) return null;
