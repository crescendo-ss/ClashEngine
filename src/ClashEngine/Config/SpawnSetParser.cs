...
/// <summary>Parses the per-team spawn-point lists nested under each game-type block:
/// <c>{prefix}Team{T}Spawns = x1,y1; x2,y2; ...</c>.</summary>
internal static class SpawnSetParser
{
    /// <summary>
    /// Reads <c>{prefix}Team{T}Spawns</c> for every team. Returns a per-team list of spawn
        ...
        IConfigManager config, string prefix, int teamCount, ClashLog? log)
    {
        ...
        // arena's default (0,0) so the orchestrator's "no spawn override" path handles it.
        for (int t = 0; t < byTeam.Count; t++)
            if (byTeam[t].Count == 0) byTeam[t].Add(default);

        var result = new IReadOnlyList<SpawnPoint>[byTeam.Count];
        for (int t = 0; t < byTeam.Count; t++) result[t] = byTeam[t];
        return result;
    }

    /// <summary>One-line summary per team for the debug log.</summary>
    public static IEnumerable<string> Describe(IReadOnlyList<IReadOnlyList<SpawnPoint>> spawnSetByTeam)
    {
        for (int t = 0; t < spawnSetByTeam.Count; t++)
        {
            var team = spawnSetByTeam[t];
    ...
    private static List<SpawnPoint> ParseList(string? raw, ClashLog? log, string prefix, string key)
    {
    ...
    private static IEnumerable<string> FormatPoints(IReadOnlyList<SpawnPoint> points)
    {
