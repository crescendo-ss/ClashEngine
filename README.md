...
| `QualityFunction` | `PredictDraw` / `OrdinalSpread` | `PredictDraw` | Engine-wide team-balance metric (the engine holds one instance, shared by every queue). `PredictDraw` (default) uses OpenSkill's own win-probability balance (`1 − |win_A − win_B|`), which folds in σ so an uncertain roster reads as closer to even. `OrdinalSpread` is the opt-out: it scores a match by how close the two teams' mean ordinal (`μ − 3σ`) is. **Scale note:** the two produce scores on different scales, so the `qStart`/`qFloor` accept bands mean different things under each. Under `PredictDraw` a band value `v` means "no worse than a `(1+v)/2 : (1−v)/2` win split" — so the standard `0.6`/`0.30` bands = start at ≤ 70/30, relax to ≤ 85/15; casual `0.4`/`0.10` = ≤ 80/20 relaxing to ≤ 95/5. Unrecognized values warn and fall back to `PredictDraw`. |
...
| `Queue<i>Preset` | `casual` | (none) | Opt-in shortcut for the lenient bundle of defaults: q-start/q-floor 0.4/0.10 (vs 0.6/0.30), `MaxMuSpread` off (vs a 10-μ cap on standard queues), `RelaxTime` 45 s, `RatingWeight` 0.5, and a slightly higher griefing threshold. Of these, `RelaxTime` and `MaxMuSpread` have their own per-queue override keys (`Queue<i>RelaxTime` / `Queue<i>MaxMuSpread`, below); q-start/q-floor and `RatingWeight` are set solely by whether this preset is on (the griefing threshold is separately settable per game type via `<GameType>TeamkillThreshold`). `MaxLiabilityGap` is off for every queue regardless of preset. Omit for the standard (stricter) defaults. |
...
| `Queue<i>MaxMuSpread` | float ≥ 0 | `10` (standard), off (`casual`) | Roster skill-eligibility cap on raw `μ`: the highest-`μ` player in a candidate match sets the bar, and anyone whose `μ` is more than this far below them is excluded from that match, so a too-weak player is never pulled into a much stronger one. `10` μ ≈ 100 leaderboard-rating points (rating ≈ 10 × (μ − 3σ)). Gates on `μ` (not ordinal), so a strong-but-uncertain player still raises the floor. Applied to the whole roster *before* teams are formed. **`0` disables** the cap for this queue; when unset the preset default applies (10 μ standard, off with `Preset = casual`). **Relaxes on wait:** once the longest waiter has been queued past `RelaxTime`, the cap is forgone so the pool can still form (the `qFloor` quality floor still applies, so a genuinely lopsided match stays blocked). |
...
| `?cancel [queue]` | player | Leave a specific queue by id (the id shown in `?queue`, the same one `?play` takes), or every queue you're in when given no id. If you're in a party, it removes the whole party from that queue (or from all queues), matching how `?play` enqueues the party. |
